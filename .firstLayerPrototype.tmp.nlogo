...
;; gladiators fight to keep the threats on point
gladiators-own[
  genes
  energy
  championship
...
to setup
  ...
  create-gladiators 150 [
    setxy random-xcor random-ycor
    set genes shuffle (n-values 53 [i -> i + 1])
    set energy 10
...
to setup-global-vars
  ...
  set importantFeatureIndex [ 7 8 9 11 12 13 14 15 16 17 18 25 26 27 28 29 30
...
to feed
  ask gladiators [
    foreach networkBehavior [ i ->
      set energy (energy + baseFood - ((position i genes) * poisonMult) - metabolismEnergy)
    ]
...
to reproduce
  ask gladiators with [energy >= 500][
    ...
    if (roulette < 8) [ ;;8% chance of giving genes
      ...
      let r1 (random 20) + 6
      let r2 (random 11) + 26
      set mutatedGenes replace-item r1 (item r2 genes)
      set mutatedGenes replace-item r2 (item r1 genes)
...
to pass-away
  ;;instead of dying, reshuffle to keep exploring new patterns
  ask gladiators with [not championship][
    if (age >= 800)[
      let prob random 101
...
false
0
Polygon -7500403 true true 150 285 15 180 60 15 240 15 285 180
